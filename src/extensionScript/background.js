	...
	switch (request.action) {
		...
		case "countBadge":
			...
			sendMessageToPortalScript({
				action: "fr_countBadge",
				content: request.count.toString(),
			});
			break;
    ...
    chrome.tabs.create(
      ...
      break;
			...
			chrome.tabs.query({ currentWindow: false, active: true }, async (tab) => {
				...
				if (request.source === "post") {
					let tabb = tab[0];
					if (tab[0].favIconUrl === undefined) tabb = tab[1];
					const postUrl = await helper.getDatafromStorage("postUrl");
					console.log("postUrl ::: ", postUrl, tab[0].id);
					chrome.tabs.create(
						{ url: postUrl, active: false, pinned: true, selected: false },
						(tabs) => {
							...
							chrome.tabs.onUpdated.addListener(async function listener(
								...
								if (info.status === "complete" && tabId === tabs.id) {
									...
									injectScript(tabs.id, ["helper.js", "storeFR.js"]);
									setTimeout(async () => {
										const feedbackTargetID = await chrome.tabs.sendMessage(
											tabs.id,
											{ action: "getFeedbackTargetID", source: "post" }
										);
										console.log(
											"FeedbackTargetID :::::::::::: ",
											feedbackTargetID
										...
										injectScript(tabb.id, ["helper.js", "storeFR.js"]);
										setTimeout(() => {
											chrome.tabs.sendMessage(tabb.id, {
												action: "start",
												source: "post",
		...
		case "shouldfrienderRun":
			...
			chrome.tabs.query({ currentWindow: true, active: true }, (tabs) => {
				...
				if (request.source === "groups") {
					if (
						tabs[0].url.includes("groups") &&
						(tabs[0].url.includes("members") || tabs[0].url.includes("people"))
					) {
						// console.log("url [groups] ::: ", tabs[0].url);
						chrome.runtime.sendMessage({ ...request, res: true });
					} else chrome.runtime.sendMessage({ ...request, res: false });
				}
				if (request.source === "suggestions") {
					if (
						tabs &&
						tabs.length &&
						tabs[0].url.includes("https://www.facebook.com/friends/") &&
						tabs[0].url.includes("suggestions")
					) {
						console.log("url [suggestions] ::: ", tabs[0].url);
						chrome.runtime.sendMessage({ ...request, res: true });
				...
				if (request.source === "friends") {
					if (
						tabs &&
						tabs.length &&
						tabs[0].url.includes("https://www.facebook.com/") &&
						tabs[0].url.includes("friends") &&
						!tabs[0].url.includes("suggestions")
					) {
						console.log("url [friends] ::: ", tabs[0].url);
						chrome.runtime.sendMessage({ ...request, res: true });
		...
		case "checkTabUrl":
			chrome.tabs.query({ currentWindow: true, active: true }, (tabs) => {
				...
				if (
					...
					setTimeout(() => {
						chrome.runtime.sendMessage({
							action: "setSettingsForFriendsOfFriend",
						});
					}, 200);
				} else if (
					tabs &&
					tabs.length &&
					tabs[0].url === "https://www.facebook.com/"
				) {
					// setTimeout(()=>{
		...
		case "getGenderCountryAndTier":
			...
			chrome.tabs.sendMessage(Number(genderTabId), {
				...request,
				responsePayload: genderCountryAndTier,
			});
			// }
			break;
		...
		case "getGenderCountryAndTierForIncoming":
			...
			list.forEach(async (el, i) => {
				const getGenderCountryAndTierForIncoming =
					await getGenderCountryAndTiers(el.friendName);
				console.log(
					"getGenderCountryAndTierForIncoming ::: ",
					getGenderCountryAndTierForIncoming
				);
				list[i] = {
					...list[i],
					gender: getGenderCountryAndTierForIncoming.gender
						? getGenderCountryAndTierForIncoming.gender
						: "N/A",
					country: getGenderCountryAndTierForIncoming.countryName
						? getGenderCountryAndTierForIncoming.countryName
						: "N/A",
					tier: getGenderCountryAndTierForIncoming.Tiers
						? getGenderCountryAndTierForIncoming.Tiers
						: "N/A",
				};
				// console.log("i ::: ", i)
				if (list.length - 1 === i)
		...
		case "sendMessage":
			...
			let { currentTime, search_date } = helper.getCurrentDayAndTimein(
				Date.now() + settingExpTime
			);
		...
		case "openPostSetting":
			chrome.tabs.query({ currentWindow: false, active: true }, async (tab) => {
				...
				if (postPopupId !== 0) {
					...
					else await helper.removeDatafromStorage("postPopupId");
				...
				chrome.windows.create(
					{ url: "popup.html", type: "popup", width: 799, height: 600 },
					async (res) => {
...
const checkScheduledSync = async () => {
	...
	let scheduleFromTime =
		settings &&
		settings.day_bak_to_analyse_friend_engagement_settings &&
	...
	scheduleFromTime = Number(
		(scheduleFromTime && scheduleFromTime.split(":"))[0]
	);
	...
	if (lastSyncDate && !helper.isEmptyObj(lastSyncDate)) {
		...
		isAlreadySyncedToday =
			todayDate.setHours(0, 0, 0, 0) === lastSyncDate.setHours(0, 0, 0, 0);
		if (isAlreadySyncedToday) {
			lastSyncId = await helper.getDatafromStorage("lastAutoSyncFriendListId");
			console.log("lastSyncId ::: ", lastSyncId, fbuserId);
			if (
				lastSyncId &&
				!helper.isEmptyObj(lastSyncId) &&
				lastSyncId.length > 0
			) {
	...
	if (
		!isAlreadySyncedToday &&
		settings &&
		settings.day_bak_to_analyse_friend_engagement &&
		timenow >= scheduleFromTime &&
		timenow < scheduleToTime
	) {
		console.log(
			"isAlreadySyncedToday:",
			isAlreadySyncedToday,
		...
		console.log(
			"Maybe syncing is already done or inprogress or not scheduled now"
		);
		console.log(
			"isAlreadySyncedToday:",
			isAlreadySyncedToday,
...
chrome.alarms.onAlarm.addListener(async (alarm) => {
	...
	switch (alarmName) {
		...
		case "InitiateSendMessages":
			...
			InitiateSendMessages(
				payload.fbDtsg,
				payload.userId,
				payload.sentFRLogForAccept,
				payload.sentFRLogForReject,
				payload.fetchIncomingLog,
				payload.fetchIncomingFRLogForAccept,
				payload.fetchIncomingFRLogForReject
			);
			break;
		...
		case "Campaign":
			...
			const nextCampaign = await helper.getDatafromStorage(
		...
		case "frQueueFetchAlarm":
			console.log(
				"-----------------Friend Request Queue Scheduler------------------",
				alarm,
				alarm.name
			);
			FrQueue_Manager();
		...
		case "frQueueRunnerAlarm":
			console.log(
				"-----------------Friend Request Queue Runner Scheduler------------------",
				alarm,
				alarm.name
			);
		...
		case "CampaignMin":
			...
			const next_campaign = await helper.getDatafromStorage(
				"Campaign_" + campaignIdd
			);
...
chrome.tabs.onUpdated.addListener(async (tabID, changeInfo, tab) => {
	...
	if (
		tab.url.includes("https://www.facebook.com/friends/") &&
		tab.url.includes("suggestions")
	) {
		CurrentTabId = await helper.getDatafromStorage("suggestedTabId");
		source = "suggestions";
	}
	if (
		tab.url.includes("https://www.facebook.com/groups/") &&
...
const markFriendRequestList = (tabID, changeInfo, tab) => {
	if (
		tab.url &&
		tab.url.startsWith("https://www.facebook.com/friends/requests") &&
		changeInfo &&
		...
		async function checkExsistingData(currFriend) {
			return new Promise(async (resolve, reject) => {
				...
				if (exFriendList.length) {
					const foundFriend = await exFriendList.find(
						(exFriendList) => exFriendList.friendFbId == currFriend.id
					);
					// console.log("foundFriend",foundFriend)
					if (foundFriend) {
						if (
							foundFriend.tier != undefined &&
							foundFriend.tier != "NA" &&
							foundFriend.country != undefined &&
							foundFriend.country != "NA" &&
							foundFriend.friendGender != undefined &&
							foundFriend.friendGender != "NA" &&
							foundFriend.friendGender != "UNKNOWN"
						) {
		...
		for (let i = 0; i < listLen; i++) {
			...
			if (exFriendData.status != 1) {
				...
				if (fetchGenderAPI.status) {
					...
					console.log(
						"GENDER from gender API",
						JSON.stringify(fetchGenderAPI.gender)
					);
					console.log(
						"conditioned final gender",
						friend.gender != undefined && friend.gender != "UNKNOWN"
							? friend.gender
							: fetchGenderAPI.gender
					);
					friendPayload.country = fetchGenderAPI.country;
					friendPayload.tier = fetchGenderAPI.tier;
					friendPayload["friendGender"] =
						friend.gender != undefined && friend.gender != "UNKNOWN"
							? friend.gender
							: fetchGenderAPI.gender;
					totalGenderAPIHits++;
			...
			if (i + 1 === listLen) {
				...
				setTimeout(() => {
					console.log(
						"totalGenderAPIhits ::",
						JSON.stringify(totalGenderAPIHits)
					);
					console.log("totalLocalfetchs ::", JSON.stringify(totalLocalfetchs));
					console.log(
						"Update country and Tier",
						JSON.stringify(updateFriendPayload)
					);
	...
	fetch(process.env.REACT_APP_FETCH_EX_FRIENDS, {
		...
		.then((res) => {
			if (
				...
				res.data != undefined &&
				res.data.length &&
				res.data[0].friend_details != undefined &&
				res.data[0].friend_details.length
			) {
				console.log(
					"friend list present***",
					JSON.stringify(res.data[0].friend_details)
				);
		...
		.catch((error) => {
			console.log(
				"error block so directly fetching gender country and tier",
				error
			);
			proceedToUpdateGenderCountryAndTier();
		});
...
const checkPendingFR = async () => {
	...
	if (settings && settings.automatic_cancel_friend_requests) {
		let settingsDetails =
			settings.automatic_cancel_friend_requests_settings &&
		...
		if (settingsDetails) {
			fbDtsg(async (fbDtsg, userID) => {
				...
				const outgoingPendingRequestDefinition =
					await helper.getOutgoingPendingRequestList(
						userID,
						true,
						settingsDetails
					);
				...
				if (
					!outgoingPendingRequestDefinition ||
					outgoingPendingRequestDefinition.length === 0
				) {
					sendMessageToPortalScript({
						type: "cookie",
						content: "Empty",
						action: "deleteAllPendingFR",
					});
					return;
				}
...
const cancelPendingFriendRequest = async (
	requestList,
	fbDtsg,
	userID,
	refriending = false
	...
	if (requestList.length > 0 && requestList[0].friendFbId) {
		...
		cancelFriendRequestDefinition = helper.makeParsable(
			cancelFriendRequestDefinition
		);
		// console.log("cancelFriendRequestDefinition :::: ", cancelFriendRequestDefinition);
		let isCancelFriendRequest =
			cancelFriendRequestDefinition &&
		...
		const isDeletedFromPortal = await helper.deleteFRFromFriender(
			[requestList[0]._id],
			userID
		);
		...
		if (refriending && isCancelFriendRequest) {
			...
			await common.sentFriendRequest(
				userID,
				fbDtsg,
				requestList[0].friendFbId,
				"groups_member_list"
			);
			const refriendingPayload = {
				...
				refriending_pending_days: Number(
					requestList[0].refriending_pending_days
				),
...
const getRefriendingList = async () => {
	...
	fbDtsg(async (fbDtsg, userID) => {
		...
		HEADERS.authorization = fr_token;
		let reFriendingList = await fetch(
			process.env.REACT_APP_REFRIENDING_URL + userID,
			{
				method: "GET",
				headers: HEADERS,
			}
		...
		if (
			reFriendingList &&
			reFriendingList.data &&
			reFriendingList.data.length > 0
		) {
		...
		(tab) => {
			chrome.tabs.onUpdated.addListener(async function listener(tabId, info) {
				if (info.status === "complete" && tabId === tab.id) {
					...
					injectScript(tab.id, ["helper.js", "collectData.js"]);
					await helper.sleep(2000);
					const userId = await chrome.tabs.sendMessage(tab.id, {
						action: "getUserId",
					});
					tab && tab.id && chrome.tabs.remove(tab.id);
					// console.log("userId ::: ", userId);
...
const sendMessageAcceptOrReject = async () => {
	...
	fbDtsg(async (fbDtsg, userId) => {
		...
		let settings = await settingResp.json();
		settings = settings && settings.data ? settings.data[0] : {};
		await helper.saveDatainStorage(
			"sendMessageOnSomeOneAccptMyFR_EvenHasConversation",
			settings &&
				settings.send_message_if_conservation_occured_when_someone_accept_new_friend_request
		);
		await helper.saveDatainStorage(
			"sendMessageOnIAccptFR_EvenHasConversation",
			settings &&
				settings.send_message_if_conservation_occured_when_accept_incoming_friend_request
		);
		// console.log("settings ::: ", settings);
		if (
			settings.send_message_when_someone_accept_new_friend_request ||
			...
			if (settings.send_message_when_someone_sends_me_friend_request) {
				fetchIncomingLog = fetchSentFRLog.filter(
					(el) =>
						el &&
			...
			if (settings.send_message_when_accept_incoming_friend_request) {
				fetchIncomingFRLogForAccept = fetchSentFRLog.filter(
					(el) =>
						...
						(el.message_sending_status !== "send" ||
							el.message_sending_setting_type !==
								settingsType.whenAcceptedByUser)
				);
				console.log(
					"fetchIncomingFRLogForAccept ::: ",
					JSON.stringify(fetchIncomingFRLogForAccept)
				);
			}
			if (settings.send_message_when_reject_incoming_friend_request) {
				fetchIncomingFRLogForReject = fetchSentFRLog.filter(
					(el) =>
						el &&
						el.friendRequestStatus &&
						el.friendRequestStatus.toLocaleLowerCase() &&
						...
						(el.message_sending_status !== "send" ||
							el.message_sending_setting_type !==
								settingsType.whenRejectedByUser)
				);
				console.log(
					"fetchIncomingFRLogForReject ::: ",
					JSON.stringify(fetchIncomingFRLogForReject)
				);
			}
			if (settings.send_message_when_someone_accept_new_friend_request) {
				fetchSentFRLogForAccept = fetchSentFRLog.filter(
					(el) =>
						el &&
						el.friendRequestStatus &&
						el.friendRequestStatus.toLocaleLowerCase() &&
						...
						(el.message_sending_status !== "send" ||
							el.message_sending_setting_type !==
								settingsType.whenAcceptedByMember)
				);
				console.log(
					"fetchSentFRLogForAccept ::: ",
					JSON.stringify(fetchSentFRLogForAccept)
				);
			}
			if (settings.send_message_when_reject_friend_request) {
				fetchSentFRLogForReject = fetchSentFRLog.filter(
					(el) =>
						el &&
						el.friendRequestStatus &&
						el.friendRequestStatus.toLocaleLowerCase() &&
						...
						(el.message_sending_status !== "send" ||
							el.message_sending_setting_type !==
								settingsType.whenRejectedByMember)
				);
				console.log(
					"fetchSentFRLogForReject ::: ",
					JSON.stringify(fetchSentFRLogForReject)
				);
			}
...
const InitiateSendMessages = async (
	fbDtsg,
	userId,
	sentFRLogForAccept = [],
	sentFRLogForReject = [],
	...
	if (sentFRLogForAccept && sentFRLogForAccept.length > 0) {
		...
		if (sentFRLogForAccept[0].message_group_request_accepted) {
			...
			console.log(
				...
				sentFRLogForAccept[0].message_group_request_accepted
			);
			message_payload = {
				fbUserId: userId,
				friendFbId: sentFRLogForAccept[0].friendFbId,
				gender: sentFRLogForAccept[0].gender
					? sentFRLogForAccept[0].gender
					: "NaN",
				country: sentFRLogForAccept[0].country
					? sentFRLogForAccept[0].country
					: "NaN",
				tier: sentFRLogForAccept[0].tier ? sentFRLogForAccept[0].tier : "NaN",
				friendName: sentFRLogForAccept[0].friendName
					? sentFRLogForAccept[0].friendName
					: "NaN",
			};
			if (sentFRLogForAccept[0].message_group_request_accepted.groupId) {
				message_payload["groupId"] =
					sentFRLogForAccept[0].message_group_request_accepted.groupId;
			} else {
				message_payload["quick_message"] =
					sentFRLogForAccept[0].message_group_request_accepted.quickMessage;
			}
		} else {
		...
		const conversationStatus = await common.checkHasConversation(
			userId,
			sentFRLogForAccept[0].friendFbId
		);
		...
		const sendMessageOnSomeOneAccptMyFR_EvenHasConversation =
			await helper.getDatafromStorage(
				"sendMessageOnSomeOneAccptMyFR_EvenHasConversation"
			);
		if (
			sendMessageOnSomeOneAccptMyFR_EvenHasConversation ||
			!conversationStatus.hasConversation
		) {
		...
		if (messageContent.status) {
			...
			storeInMsqs(
				userId,
				sentFRLogForReject[0].friendFbId,
				sentFRLogForReject[0].friendName,
				messageContent.content,
				settingsType.whenRejectedByMember,
				exp_time,
				campaign_send_date
		...
		if (messageContent.status) {
			...
			storeInMsqs(
				userId,
				fetchIncomingLog[0].friendFbId,
				fetchIncomingLog[0].friendName,
				messageContent.content,
				settingsType.whenRecievesRequest,
				exp_time,
				campaign_send_date
		...
		fetchIncomingFRLogForAccept &&
		fetchIncomingFRLogForAccept.length > 0
	) {
		const message_payload = {
			fbUserId: userId,
		...
		const conversationStatus = await common.checkHasConversation(
			userId,
			fetchIncomingFRLogForAccept[0].friendFbId
		);
		// send message to user
		// console.log("sendMessageOnIAccptFR_EvenHasConversation",sendMessageOnIAccptFR_EvenHasConversation);
		// console.log("has converse in i am accepting",conversationStatus);
		const sendMessageOnIAccptFR_EvenHasConversation =
			await helper.getDatafromStorage(
				"sendMessageOnIAccptFR_EvenHasConversation"
			);
		if (
			...
			if (messageContent.status) {
				...
				storeInMsqs(
					userId,
					fetchIncomingFRLogForAccept[0].friendFbId,
					fetchIncomingFRLogForAccept[0].friendName,
					messageContent.content,
					settingsType.whenAcceptedByUser,
					exp_time,
					campaign_send_date
		...
		fetchIncomingFRLogForReject &&
		fetchIncomingFRLogForReject.length > 0
	) {
		const message_payload = {
			fbUserId: userId,
		...
		if (messageContent.status) {
			...
			storeInMsqs(
				userId,
				fetchIncomingFRLogForReject[0].friendFbId,
				fetchIncomingFRLogForReject[0].friendName,
				messageContent.content,
				settingsType.whenRejectedByUser,
				exp_time,
				campaign_send_date
		...
		fetchSentFRLogForAcceptFromFrQueue &&
		fetchSentFRLogForAcceptFromFrQueue.length > 0
	) {
		const message_payload = {
			fbUserId: userId,
		...
		if (messageContent.status) {
			...
			storeInMsqs(
				userId,
				fetchSentFRLogForAcceptFromFrQueue[0].friendFbId,
				fetchSentFRLogForAcceptFromFrQueue[0].friendName,
				messageContent.content,
				settingsType.whenAcceptedByUser,
				exp_time,
				campaign_send_date
	...
	if (
		sentFRLogForAccept.length ||
		sentFRLogForReject.length ||
		fetchIncomingLog.length ||
		fetchIncomingFRLogForAccept.length ||
		fetchIncomingFRLogForReject.length
	) {
		payload = {
			fbDtsg: fbDtsg,
			userId: userId,
...
async function MSQS() {
	...
	if (dtsg && fr_fbId) {
		removeFromQueue(async function (queueInfo) {
			if (queueInfo) {
				if (queueInfo.notCampaignMessage) {
					...
					if (fr_fbId == fbId) {
						if (
							...
							if (queueInfo.campaignId) {
								...
								if (
									messageStatus &&
									messageStatus.message_status_boolean === 0 &&
									messageStatus.remaining_limit_count > 0
								) {
									let fbResponse = await sendMessageViaFacebook(
										dtsg,
										queueInfo
									);
									...
									console.log(
										"Message not sent as its already been sent or the limit is exceeded (campaign automation)",
										messageStatus
									);
								}
								...
								// Settings automation
								const messageStatus = await common.checkMessageStatus(
									queueInfo.recieverFbId,
									queueInfo.settingsType
								);
								if (
									messageStatus &&
									messageStatus.message_status_boolean === 0
						...
						console.log(
							"added back to queue as current fbid and camapign fb id is not matching",
							queueInfo
						);
					...
					MSQS();
					console.log(
						"The data has been removed from queue as its expired",
						queueInfo
					);
				}
			} else {
		...
		console.log(
			"FB id and dtsg not found in local storage, So MSQS is not executing"
		);
	}
}
...
let sendMessageViaFacebook = async (dtsg, queueInfo, alt = false) => {
	try {
		if (queueInfo.message.trim() === "") return false;
		else {
			...
			if (!alt && response.includes("You cannot perform that action")) {
				...
				console.log(
					"Successfully resolved the alternate message sending technique"
				);
				return true;
			}
		}
...
async function handleResponse(queueInfo, response) {
	try {
		...
		if (
			queueInfo &&
			queueInfo.campaignId != undefined &&
			queueInfo.campaignId
		) {
...
const startCampaignScheduler = async () => {
	...
	if (!helper.isEmptyObj(helper.getDatafromStorage("fr_token"))) return;
	//clear storage
	...
	let alarms = await chrome.alarms.getAll();
	let alarms_campaign =
		alarms && alarms.filter((el) => el.name.includes("Campaign_"));
	...
	let alarms_campaign_min =
		alarms && alarms.filter((el) => el.name.includes("CampaignMin_"));
	...
	allCampaigns =
		allCampaigns && allCampaigns.filter((el) => el.status === true);
	// console.log("allCampaigns ::: ", allCampaigns);
	const { day, currentTime } = helper.getCurrentDayAndTimein();
	allCampaigns &&
		allCampaigns.forEach((el, i) => {
...
const startSchedulerOfSubCampaign = async (day, currentTime, campaign) => {
	...
	campaign &&
		campaign.schedule &&
		campaign.schedule.forEach(async (elem, indx) => {
			...
			if (elem.day === day) {
				...
				if (curr_hour > scheduled_hour && curr_hour < scheduled_end_hour) {
					...
					curr_hour === scheduled_end_hour &&
					curr_minute <= scheduled_end_minute
				) {
					upcomingDateAndTime = Date.now() + 60 * 1000;
				} else if (
					curr_hour === scheduled_hour &&
					curr_minute >= scheduled_minute
				) {
					upcomingDateAndTime = Date.now() + 60 * 1000;
				} else if (curr_hour < scheduled_hour) {
					upcomingDateAndTime =
						Date.now() +
						60 * diff_hour * 60 * 1000 +
						60 * diff_min * 1000 +
						diff_sec * 1000 +
						60 * 1000;
					...
					upcomingDateAndTime =
						Date.now() +
						gapBetweenTwoDays * 24 * 60 * 60 * 1000 +
						60 * diff_hour * 60 * 1000 +
						60 * diff_min * 1000 +
						diff_sec * 1000 +
						60 * 1000;
					...
					upcomingDateAndTime =
						Date.now() + 60 * diff_min * 1000 + diff_sec * 1000 + 60 * 1000;
				} else if (
					curr_hour === scheduled_end_hour &&
					curr_minute > scheduled_end_minute
				) {
			...
			chrome.alarms.create("Campaign_" + campaign.campaign_id + "_" + indx, {
				when: upcomingDateAndTime,
			});
		});
};
...
const satrtHourlyScheduler = async (campaign, indx) => {
	...
	const campaignStatus = await common.checkCampaignStatus(
		userID,
		day,
		currentTime,
	...
	if (campaignStatus && campaignStatus.status === "Active") {
		...
		if (campaign.campaign_contacts && campaign.campaign_contacts.length > 0) {
			...
			allCampaigns = await common.fetchAllCampaignList(
				day,
				currentTime,
				search_date
			); // fetch campaign
			allCampaigns =
				...
				allCampaigns.filter((el) => campaign.campaign_id === el._id);
			// console.log("allCampaigns ::: ", allCampaigns);
		...
		await helper.saveDatainStorage(
			"Campaign_" + campaign.campaign_id,
			campaign
		);
		chrome.alarms.create("CampaignMin_" + campaign.campaign_id + "_" + indx, {
			periodInMinutes: campaign.time_delay,
		});
	} else {
...
const campaignToMsqs = async (campaign, indx) => {
	...
	if (campaignStatus && campaignStatus.status === "Active") {
		...
		let anyDuplicateData =
			messageQueue &&
			messageQueue.filter(
				(el) =>
					el.recieverFbId === campaign.campaign_contacts[0].friendFbId &&
					el.campaignId === campaign.campaign_id
			);
		if (anyDuplicateData && anyDuplicateData.length === 0) {
			...
			const message_payload = {
				...
				quick_message: campaign.quick_message
					? campaign.quick_message.messengerText
					: null,
			};
			// check member status
			const messageContent = await common.getMessageContent(message_payload);
			if (messageContent.status) {
				...
				storeInMsqs(
					campaign.fb_user_id,
					campaign.campaign_contacts[0].friendFbId,
					campaign.campaign_contacts[0].friendName,
					messageContent.content,
					7,
					exp_time,
					campaign_send_date,
					campaign.campaign_id,
					campaign.campaign_contacts[0]._id,
					message_payload.groupId
				);
			}
		}
...
async function frQueueSentCountInit(userFbId, frQueSettings) {
	...
	if (frCurrQueueCount && frQueLocalSettings) {
		...
		if (
			(!frQueLocalSettings.runningStatus && frQueSettings.run_friend_queue) ||
			frCurrQueueCount.userFbId !== userFbId
		) {
			let countObj = {
				count: 0,
...
const FrQueue_Manager = async (callFromFetchAlarm = false) => {
	...
	if (frQueSettings && frQueSettings.run_friend_queue) {
		// * if the Run true:>
		//Init run counter
		let countInitRes = await frQueueSentCountInit(
			userFBDetails.userID,
			frQueSettings
		);
		...
		const frQueueResp = await common.fetchTopPriorityFrQueueRecords(
			userFBDetails.userID
		);
		...
		if (frQueArray.length > 0) {
			console.log(
				`${frQueArray.length} Items Added to FR queue storage`,
				frQueArray
			);
			...
			console.log(
				"frQueueRunnerAlarm  started DELAY::: ",
				frQueSettings.time_delay
			);
...
const createAndinjectScript = (profileUrl) => {
	return new Promise((resolve, reject) => {
		chrome.tabs.create(
			{ url: profileUrl, active: false, pinned: true, selected: false },
			(tab) => {
...
const profileSettingCheck = async (userID, memberId) => {
	...
	profileMysettings =
		profileMysettings && profileMysettings.data
			? profileMysettings.data[0]
			: {};
	console.log("profile setting", profileMysettings);
	if (
		profileMysettings &&
		profileMysettings.dont_send_friend_requests_prople_ive_been_friends_with_before
	) {
	...
	if (
		...
		const isRejectedFriends = await helper.fetchRejectedFriends(
			userID,
			memberId
		);
		...
		if (
			...
			if (
				isRejectedFriends &&
				isRejectedFriends.isRejected &&
				!isRejectedFriends.is_incoming
			)
				return false;
		}
		if (
	...
	if (
		profileMysettings &&
		profileMysettings.avoid_sending_friend_request_to_restricted_people
	) {
		const isRestricted = await common.fetchRestrictedFbProfile({
...
const runFriendRequestQueue = async () => {
	...
	let updatePayload = {
		...
		friendProfileUrl:
			first.friendProfileUrl.length > 0
				? first.friendProfileUrl
				: `https://www.facebook.com/profile.php?id=${first.friendFbId}`,
	};
	if (first && isEligible) {
		...
		try {
			...
			if (first.friendFbId.length > 0) {
				...
				const settingEligibility = profileSettingCheck(
					userFBDetails.userID,
					first.friendFbId
				);
				...
				profileInfo = await common.getProfileInfo(
					first.friendFbId,
					userFBDetails.fbDtsg,
					userFBDetails.userID
				);
				await helper.sleep(7000);
				if (profileInfo)
					sentFrReqResponse = await common.sentFriendRequest(
						userFBDetails.userID,
						userFBDetails.fbDtsg,
						first.friendFbId,
						"profile_button",
				...
				const userData = await chrome.tabs.sendMessage(create.tabId, {
					action: "getUserFbId",
					tabId: create.tabId,
				});
				...
				const sendFrReqDomParseResp = await chrome.tabs.sendMessage(
					create.tabId,
					{ action: "clickAddFriend", tabId: create.tabId }
				);
				console.log("dom parse response", sendFrReqDomParseResp);
				await helper.sleep(5000);
				const cancleBtn = await chrome.tabs.sendMessage(create.tabId, {
					action: "checkCancelBtn",
					tabId: create.tabId,
				});
				...
				await chrome.tabs.sendMessage(create.tabId, {
					action: "closeTab",
					tabId: create.tabId,
				});
				...
				if (!cancleBtn.status) {
					...
					throw new Error(
						"With the current url we can't send FriendRequest by DOM parsing"
					);
				}
				sentFrReqResponse.status = sendFrReqDomParseResp.status;
				console.log("sent DOM response: ::::::>>>>>", sendFrReqDomParseResp);
				profileInfo = {
					fbUserId: helper.removeExtraQuotes(
						sendFrReqDomParseResp.fbUserId ? sendFrReqDomParseResp.fbUserId : ""
					),
			...
			if (!sentFrReqResponse.status) {
				...
				const friendGenderCountrytier =
					await common.getGenderCountryTierWithName(profileInfo.name);
				console.log(
					"friendGenderCountrytier***************",
					friendGenderCountrytier
				);
				console.log(
					"friend request sending done for userID: ",
					first.friendProfileUrl
				);
				...
				updatePayload["friendFbId"] =
					first.friendFbId.length > 0 ? first.friendFbId : profileInfo.fbUserId;
				...
				if (first.message_group_request_sent) {
					...
					const friendDetails = {
						fbUserId:
							first.friendFbId && first.friendFbId.length > 0
								? first.friendFbId
								: profileInfo.fbUserId,
					...
					if (first.message_group_request_sent.groupId.length > 0) {
						...
						friendDetails["quick_message"] =
							first.message_group_request_sent.quickMessage;
					}
		...
		console.log(
			":::KILLING the fr queue AS there is No data  OR NOT ELIGIBLE in local QUEUE !!!!!"
		);
		frQue_Kill();
		//stop the process
	}

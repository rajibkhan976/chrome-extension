			...
			if (cursor) {
				if (groupSettings.reaction) {
					variables = {
						...
						reactionID: reactions[groupSettings.reaction_type[0]],
						scale: 1,
						id: feedbackTargetID,
					};
				}
...
const getContactList = async (source, cursor = null) => {
	...
	if (shoudIstop) return;
	let memberlist = await fetch("https://www.facebook.com/api/graphql/", {
		method: "POST",
		headers: {
	...
	if (source === "suggestions") {
		...
		page_info =
			memberlist &&
			memberlist.people_you_may_know &&
			memberlist.people_you_may_know.page_info;
		memberlist =
			memberlist &&
			memberlist.people_you_may_know &&
			memberlist.people_you_may_know.edges;
	...
	if (source === "groups") {
		if (cursor) {
			...
			let memberArr = [];
			memberlist = memberlist && memberlist.data && memberlist.data.group;
			page_info =
				memberlist &&
			...
			if (
				memberlist.paginated_member_sections[0] &&
				memberlist.paginated_member_sections[0].__typename ===
					"GroupContributorsSection"
			)
				memberArr = [
					...memberArr,
					...memberlist.paginated_member_sections[0].group.contributors.edges,
				];
			if (
				memberlist.paginated_member_sections[1] &&
				memberlist.paginated_member_sections[1].__typename ===
					"GroupThingsInCommonSection"
			)
				memberArr = [
					...memberArr,
					...memberlist.paginated_member_sections[1].group
						.group_member_discovery.edges,
				];
			if (
				memberlist.paginated_member_sections[2] &&
				memberlist.paginated_member_sections[2].__typename ===
					"GroupThingsInCommonSection"
			)
				memberArr = [
					...memberArr,
					...memberlist.paginated_member_sections[2].group
						.group_member_discovery.edges,
				];
		...
		if (source === "post") {
			...
			if (!groupSettings.reaction && groupSettings.comment) {
				can_request =
					response[el] &&
					response[el].node &&
					response[el].node.author.subscribe_status === "CAN_SUBSCRIBE"
						? "CAN_REQUEST"
						: "CAN_SUBSCRIBE";
			}
			if (groupSettings.reaction) {
			...
			if (source === "suggestions") {
				arr = [
					...arr,
					{
						...
						mutual_friend:
							response[el].node.social_context.text.split(" mutual ")[0] === ""
								? 0
								: response[el].node.social_context.text.split(" mutual ")[0],
			...
			if (source === "friends") {
				arr = [
					...arr,
					{
						...
						mutual_friend:
							response[el].node.subtitle_text.text.split(" mutual ")[0] === ""
								? 0
								: response[el].node.subtitle_text.text.split(" mutual ")[0],
						sourceUrl: window.location.href,
			...
			if (source === "groups") {
				arr = [
					...arr,
					{
						...
						finalSource: source,
						groupKeywords: [
							response[el].node.name,
							response[el].join_status_text
								? response[el].join_status_text.text
								: "",
							...
							response[el].node.timeline_context_items
								? response[el].node.timeline_context_items.edges
									? response[el].node.timeline_context_items.edges[0]
										? response[el].node.timeline_context_items.edges[0].node
											? response[el].node.timeline_context_items.edges[0].node
													.title.text
											: ""
										: ""
					...
					arr = [
						...arr,
						{
							...
							friendProfilePicture:
								response[el].node.author.profile_picture_depth_0.uri,
							groupKeywords: [
								response[el].node.author.name,
								response[el].node.body.text,
							],
							sourceUrl: sourceUrl,
							profile_viewed: memberCount,
							time_saved: "",
